...
export const lessonThaiTranslations = {
  ...
  "typescript/type-inference-boundaries": {
    codeComments: {
      goodCode: ["กำหนด type ที่จุดรับข้อมูลเข้า แล้วให้ค่าข้างใน infer จากโค้ดจริง"],
      badCode: ["ปล่อย input เป็น any แม้จะใส่ type ให้ตัวแปรข้างใน จุดเสี่ยงจึงยังเปิดอยู่"],
    },
    ...
    takeaways: ["กำหนด type ที่ parameter, API response, form data และ API ที่ export ตั้งแต่จุดรับข้อมูลเข้า ส่วนค่าภายในให้ TypeScript infer จากโค้ดจริงได้."],
    whatToReview: [
      "โค้ดที่ดีระบุ type ตรงจุดที่ข้อมูลเข้าฟังก์ชันหรือ adapter ก่อนส่งต่อเข้า logic ด้านใน.",
      "โค้ดที่ควรปรับปล่อย input จาก caller เป็น any แม้จะใส่ type ให้ตัวแปรข้างใน ทำให้ข้อมูลผิดรูปยังไหลเข้าได้.",
    ],
    reviewNotes: [
      "เวลารีวิวให้ดูจุดที่ข้อมูลไหลเข้าระบบก่อน เช่น exported function, API adapter, form handler และ callback ที่ส่งข้าม component.",
    ],
  },
  "typescript/unsafe-assertions": {
    codeComments: {
      goodCode: ["ตรวจค่าตอนรันจริงก่อนยอมรับว่าเป็น Session"],
      badCode: ["บอกให้ TypeScript เชื่อว่า unknown เป็น Session โดยไม่ได้ตรวจค่าจริง"],
    },
    ...
    takeaways: ["ใช้ as เฉพาะหลังตรวจข้อมูลจริง หรือใน boundary ที่รู้รูปข้อมูลแน่นอน ไม่ใช่เพื่อซ่อน error ใน editor."],
    whatToReview: [
      "โค้ดที่ดีตรวจ userId และ expiresAt ตอนรันจริงก่อนคืนค่าเป็น Session.",
      "โค้ดที่ควรปรับใช้ as กับข้อมูลจากภายนอกทันที ทำให้ข้อมูลผิดรูปถูกส่งต่อเหมือนเป็น Session.",
    ],
    reviewNotes: [
      "เวลาเห็น `as` ใกล้ข้อมูลจาก JSON, storage, network, URL params หรือ library ภายนอก ให้ถามว่าค่าถูกตรวจมาก่อนหรือยัง.",
    ],
  },
  "typescript/exhaustive-never-checks": {
    codeComments: {
      goodCode: ["ใช้ never ทำให้ union case ที่ตกหล่นกลายเป็น compile error"],
      badCode: ["fallback กว้างซ่อน state ใหม่ที่ยังไม่มี branch"],
    },
    ...
    takeaways: ["ใช้ never ใน branch สุดท้ายเพื่อตรวจว่า union ทุกสถานะมี branch ของตัวเองแล้ว."],
    whatToReview: [
      "โค้ดที่ดีทำให้ branch สุดท้ายรับค่าไม่ได้เมื่อเขียนครบทุกสถานะ และจะเตือนเมื่อมีสถานะใหม่เพิ่ม.",
      "โค้ดที่ควรปรับใช้ fallback กว้างจนสถานะใหม่ถูกแสดงเป็น Unknown แทนที่จะมี branch ของตัวเอง.",
    ],
    reviewNotes: [
      "default branch ของ union อาจซ่อนงานที่ยังไม่ได้เขียน. ถ้ารู้ทุกสถานะอยู่แล้ว ให้ใช้ never เป็นตัวเตือนตอนเพิ่มสถานะใหม่.",
    ],
  },
  "typescript/optional-vs-nullable": {
    codeComments: {
      goodCode: ["null แปลว่า avatar ตั้งใจว่าง ไม่ใช่ field หายไป"],
      badCode: ["optional ปน null ทำให้คนเรียกเดาว่าค่าว่างหมายถึงกรณีไหน"],
    },
    ...
    takeaways: ["แยก `undefined`, `null` และ `\"\"` ให้มีความหมายคนละแบบ เพื่อให้ API และ form ไม่ต้องเดาค่าว่างซ้ำ."],
    whatToReview: [
      "โค้ดที่ดีแยก field ที่ยังไม่ได้ส่งมาออกจาก field ที่ตั้งใจไม่มี avatar จาก type ได้ทันที.",
      "โค้ดที่ควรปรับยอมทั้ง undefined และ null แทบทุกที่ ทำให้ caller แยกไม่ออกว่าข้อมูลหาย ถูกล้าง หรือยังโหลดไม่เสร็จ.",
    ],
    reviewNotes: [
      "ค่าว่างเป็นกฎของ domain model. `undefined`, `null` และข้อความว่างควรถูกใช้คนละความหมายก่อนส่งข้อมูลข้าม API หรือ form.",
    ],
  },
  "typescript/generic-constraints": {
    codeComments: {
      goodCode: ["extends บอกว่า generic ต้องมี field ที่ function ใช้"],
      badCode: ["any ซ่อน field ที่หายไป จน error ไปเกิดตอน runtime"],
    },
    ...
    takeaways: ["ใช้ extends เมื่อ generic ต้องอ่าน field อย่าง id หรือ updatedAt เพื่อให้ function ยืดหยุ่นแต่ยังตรวจ type ได้."],
    whatToReview: [
      "โค้ดที่ดียังรับ item ได้หลายรูปแบบ แต่ระบุว่าทุก item ต้องมี updatedAt เป็นเงื่อนไขขั้นต่ำ.",
      "โค้ดที่ควรปรับรับ type อะไรก็ได้แล้ว cast เป็น any ข้างใน ทำให้เรียกผิดแล้วยัง compile ผ่าน.",
    ],
    reviewNotes: [
      "Generic ที่ cast เป็น any ทันทีมักยังไม่ได้บอกเงื่อนไขของข้อมูลให้พอ ให้ถามว่า field ไหนจำเป็นต่อ function นี้.",
    ],
  },
  "typescript/utility-types-api-boundaries": {
    codeComments: {
      goodCode: ["Pick เลือกเฉพาะ field ที่ list API ต้องส่งออก"],
    ...
    takeaways: ["ใช้ Pick, Omit, Partial และ Required เพื่อทำ type สำหรับ request, form หรือ view ให้เหลือเฉพาะ field ที่ต้องใช้."],
    whatToReview: [
      "โค้ดที่ดีเปิดเผยเฉพาะ id, title และ authorId ตามสัญญาของ list API โดยยังผูกกับ PullRequest หลัก.",
      "โค้ดที่ควรปรับคืน PullRequest ทั้งก้อน รวมถึง mergedAt และ internalScore ที่ boundary ไม่ควรเปิดเผย.",
    ],
    reviewNotes: [
      "Utility types ควรบอกสัญญาของ boundary ว่า request, form หรือ view ใช้ field ไหน ไม่ใช่แค่ย่อ type ให้สั้นลง.",
    ],
  },
  "typescript/type-only-imports-module-boundaries": {
    codeComments: {
      goodCode: ["import type บอกว่า PullRequest ใช้เฉพาะตอนตรวจ type"],
      badCode: ["import ปกติทำให้ type dependency ดูเหมือน runtime dependency"],
    },
    ...
    takeaways: ["ใช้ import type เมื่อ import นั้นมีไว้ตรวจ type เท่านั้น และใช้ import ปกติเมื่อ code ต้องมีค่าจริงตอน runtime."],
    whatToReview: [
      "โค้ดที่ดีแยก PullRequest เป็น type dependency และแยก formatAuthorName เป็น runtime dependency.",
      "โค้ดที่ควรปรับ import type เหมือนค่าที่ใช้ตอน runtime ทำให้ module boundary อ่านยาก และอาจดึง code เข้ามาเกินจำเป็น.",
    ],
    reviewNotes: [
      "Import บอกว่าไฟล์นี้พึ่งอะไร ถ้าใช้แค่รูปข้อมูล ให้ใช้ import type เพื่อแยก type ที่ใช้ตอนเขียนโค้ดออกจากค่าที่รันจริง.",
    ],
  },
  "typescript/discriminated-unions": {
    codeComments: {
      goodCode: ["field status บอกว่า branch นี้ใช้ field ชุดไหนได้"],
      badCode: ["optional field หลายตัวทำให้สถานะที่เป็นไปไม่ได้ compile ผ่าน"],
    },
    ...
    takeaways: ["ใช้ field กลางอย่าง status, type หรือ kind เพื่อแยกรูปข้อมูลของแต่ละสถานะ."],
    whatToReview: [
      "โค้ดที่ดีมี field กลางสำหรับบอกสถานะ ทำให้ TypeScript รู้ว่า branch นี้อ่าน id หรือ message ได้.",
      "โค้ดที่ควรปรับใช้ optional field หลายตัวจน caller ต้องเดาเองว่า field ไหนควรมาคู่กัน.",
    ],
    reviewNotes: [
      "เวลารีวิว type ให้ถามว่า object มีสถานะที่ไม่ควรเกิดขึ้นแต่ type ยังยอมไหม. union ที่มี field บอกสถานะช่วยปิดช่องนั้น.",
    ],
  },
  "typescript/narrowing-unknown": {
    codeComments: {
      goodCode: ["type guard ตรวจชนิดและ field ก่อนคืนค่าเป็น Settings"],
      badCode: ["double cast เปลี่ยน unknown เป็น Settings โดยไม่ตรวจค่าจริง"],
    },
    ...
    takeaways: ["unknown คือข้อมูลที่ยังไม่รู้ชนิด ควรตรวจ type และ field ที่ต้องใช้ก่อนส่งต่อเข้า logic."],
    whatToReview: [
      "โค้ดที่ดีตรวจว่า value เป็น object และมี field ที่ต้องใช้ก่อนนำไปใช้ต่อ.",
      "โค้ดที่ควรปรับ cast เป็น any หรือ double cast ทำให้ TypeScript หยุดช่วยตรวจจุดที่ข้อมูลยังไม่น่าเชื่อถือ.",
    ],
    reviewNotes: [
      "ข้อมูลจาก API, localStorage หรือ message event ควรถูกตรวจก่อนใช้เสมอ. ให้มองหาจุดที่ unknown ถูกแปลงเป็น type ของระบบโดยไม่มี guard.",
    ],
  },
  "typescript/precise-function-types": {
    codeComments: {
      goodCode: ["callback type บอก action, argument และ return ที่เรียกได้"],
      badCode: ["Function กว้างเกินไป ทำให้รูปแบบการเรียกผิดยัง compile ผ่าน"],
    },
    ...
    takeaways: ["กำหนด parameter และ return type ของ function ให้เห็น contract โดยเฉพาะ callback ที่ส่งต่อให้ส่วนอื่นใช้."],
    whatToReview: [
      "โค้ดที่ดีบอก action, argument และ return type ของ callback ตามรูปการเรียกจริง.",
      "โค้ดที่ควรปรับใช้ Function กว้าง ๆ ทำให้ caller ส่ง argument ผิดหรือ action นอกชุดที่รองรับได้.",
    ],
    reviewNotes: [
      "เวลาเห็น Function ใน TypeScript ให้ถามต่อว่า function นี้รับค่าอะไรและคืนค่าอะไร. ส่วนใหญ่ contract ควรถูกเขียนเป็น signature เฉพาะ.",
    ],
  },
